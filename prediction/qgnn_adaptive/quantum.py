...
def quantum_round(state,ry,rz,pa,ta,rx,mask):
    ...
    for i in range(n):state=rotate(state,(rx[:,i] if rx.ndim==2 else rx)*mask[:,i],i,"X")
    return state

class SceneAdaptiveQuantumCore(RelationCarryingQuantumCore):
    def __init__(self,depth=3,channels=4,controller_seed=802029,feedback=True,phase_mode=False,basis_feedback=False):
        super().__init__(depth,channels)
        with torch.random.fork_rng(devices=[]):
            torch.manual_seed(controller_seed);self.controller=CouplingController(depth,channels)
        self.feedback_enabled=feedback;self.phase_mode=phase_mode;self.basis_feedback=basis_feedback
        if basis_feedback:
    ...
    def quantum_states(self,history,mask,trace=False):
        ...
        for layer in range(self.depth):
            ...
            if self.basis_feedback and layer>0 and self.feedback_enabled and self.controller.enabled:
                nf=node_feedback(feedback,d,n)
                delta=.6*torch.tanh((nf*self.basis_weights[:,layer-1][None,:,None]).sum(-1))
                mix=mix[:,None]+delta.reshape(b*ch,n)
            ...
            if layer<self.depth-1 or trace:
                ...
                if trace:logs.append(dict(lambda2=l2,lambda3=l3,k2=k2,k3=k3,pair_phase=p,triple_phase=t,rx_angles=mix))
