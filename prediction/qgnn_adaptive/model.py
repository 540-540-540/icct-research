...
def build_model(kind,seed=2026,depth=3,token_path=None,channels=4,enhanced=True,snr_db=0.,quantum_version=3,correction_cap_m=16.,adaptive_mode="feedback"):
    if adaptive_mode not in ("feedback","history","phase_feedback"):raise ValueError(adaptive_mode)
    ...
    with torch.random.fork_rng(devices=[]):
        ...
        core=cls(depth,channels,seed+800003,adaptive_mode!="history",**({"phase_mode":adaptive_mode=="phase_feedback"} if kind=="quantum" else {}))
    with torch.random.fork_rng(devices=[]):
