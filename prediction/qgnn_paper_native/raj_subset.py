...
class RajSubsetQGNNCore(BoundedCore):
    def __init__(self,j=3,D=6,k=3,rounds=3,encoder_hidden=64):
        super().__init__();self.j=j;self.rounds=rounds
        import math
        self.embed_dim=math.comb(D,k);self.builder=SubsetFeatureBuilder(32);f=self.builder.out_dim
        ...
        self.alphas=nn.Parameter(torch.randn(rounds)*.3)
    ...
    def forward_patch(self,history,mask):
        ...
        for r in range(self.rounds):
            x=self._mix(x,self.alphas[r],own.shape[1])*valid[...,None]
            x=x+self._unit(self.encs[r](feat)).to(x.dtype)*valid[...,None]
...
class RajMultiJQGNNCore(BoundedCore):
    """Both j=2 and j=3 are quantum subset branches in every scene."""
    def __init__(self,rounds=3):
        super().__init__();self.j2=RajSubsetQGNNCore(j=2,rounds=rounds);self.j3=RajSubsetQGNNCore(j=3,rounds=rounds);self.fuse=MultiJFusion()
    def forward_patch(self,history,mask):
