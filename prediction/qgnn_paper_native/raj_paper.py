...
class RajPaperQGNNCore(BoundedCore):
    """Two-register reduced-basis Raj-style QGNN adaptation."""

    def __init__(self,j=3,depth=3,D=6,k=3):
        super().__init__();self.j=j;self.depth=depth;self.D=D;self.k=k
        self.loader=ContinuousSubsetLoader(32,D,k,64)
        self.mix=WeightedJohnsonMix(j,depth)
        self.embed_layers=nn.ModuleList(CompoundEvolution(D,k) for _ in range(depth))
        self.reupload=nn.ModuleList(nn.Sequential(nn.Linear(self.loader.feat_dim,64),nn.SiLU(),nn.Linear(64,comb(D,k))) for _ in range(depth))
        self.readout=IncidenceRDMReadout(D,64)

    def forward_patch(self,history,mask):
        own,edge,risk,feat,amp,valid,subs=self.loader(history,mask,self.j)
        x=amp.to(torch.complex64 if amp.dtype==torch.float32 else torch.complex128)
        for l in range(self.depth):
            x=self.mix(x,edge,mask,l)
            x=self.embed_layers[l](x)
            up=self.reupload[l](feat)
            up=up/(torch.linalg.vector_norm(up,dim=-1,keepdim=True)+1e-8)
            x=(x+up.to(x.dtype))*valid[...,None]
            x=x/(torch.linalg.vector_norm(x,dim=-1,keepdim=True)+1e-8)
            x=x*valid[...,None]
        return self.readout(x,valid,subs,history.shape[2],mask)


class RajPaperJohnsonGINCore(BoundedCore):
    """Paper-native matched classical subset/Johnson message-passing baseline."""

    def __init__(self,j=3,depth=3,hidden=128):
        super().__init__();self.j=j;self.depth=depth;self.hidden=hidden
        self.loader=ContinuousSubsetLoader(32,6,3,64)
        self.edge_weight=WeightedJohnsonMix(j,depth)
        self.input=nn.Sequential(nn.Linear(self.loader.feat_dim,hidden),nn.SiLU(),nn.Linear(hidden,hidden))
        self.layers=nn.ModuleList(nn.Sequential(nn.Linear(2*hidden,hidden),nn.SiLU(),nn.Linear(hidden,hidden)) for _ in range(depth))
    ...
    def forward_patch(self,history,mask):
        own,edge,risk,feat,amp,valid,subs=self.loader(history,mask,self.j)
        h=self._unit(self.input(feat))*valid[...,None];hs=[h]
        ...
        _,_,inc0=subset_tensors(history.shape[2],self.j);inc=inc0.to(h.device,h.dtype)
...
class RajWeightedSubsetQGNNCore(BoundedCore):
    ...
    def __init__(self,j=3,rounds=3,D=6,k=3,encoder_hidden=64):
        ...
        from .raj_subset import SubsetFeatureBuilder,_SubsetReadout
...
class RajWeightedMultiJQGNNCore(BoundedCore):
    ...
    def __init__(self,rounds=3):
        super().__init__()
        from .raj_subset import MultiJFusion
