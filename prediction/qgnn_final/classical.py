...
def weighted_attention(value,score,weight,valid):
    score = score+torch.log(weight.clamp_min(1e-4))
    ...
    return (alpha[...,None]*value).sum(-2)

class AdaptiveHypergraphLayer(nn.Module):
    def __init__(self,width=64):
        super().__init__()
        self.pair = nn.Sequential(nn.Linear(2*width+16,128),nn.SiLU(),nn.Linear(128,width+1))
        self.triplet = nn.Sequential(nn.Linear(3*width+48,128),nn.SiLU(),nn.Linear(128,width+1))
...
class AdaptiveClassicalCore(BoundedCore):
    def __init__(self,depth=3):
        ...
        self.layers = nn.ModuleList([AdaptiveHypergraphLayer() for _ in range(depth)])
