...
def make_main_comparison(df: pd.DataFrame, out_dir: Path) -> dict:
    ...
    legend = [
        ...
        Patch(facecolor="#F6E8D9", edgecolor="none", label="Proposed method row"),
    ]
    fig.legend(
        handles=legend,
        loc="upper center",
...
def make_accuracy_safety(df: pd.DataFrame, out_dir: Path) -> dict:
    ...
    for row in learned.itertuples(index=False):
        ...
        ax.annotate(
            ...
            fontsize=7,
            color=color if row.model_key in {"target_interaction_gnn", "graph_motion_token_gpt2"} else "black",
        )
