...
| `T` | 12.375 µs | 无模糊速度 `c/(4fc·T)` = 252.35 m/s；CPI = `N·T` = 3.168 ms |
| `c` | 299 792 458 m/s | — |

已知简化（继承审计结论并显式声明）：`T` 为冻结的符号间隔，不模拟 CP/ICI；`T` 与 `df` 独立声明。
...
## 4. 单 BS 处理链（Range–Doppler–Angle）
...
angle:   矩形窗(a) → FFT(A=16→Na=64) 并 fftshift   u 栅格 0.03125
P_b[r,v,u] = |S_b[r,v,u]|²                         3D 功率立方，shape [512,512,64]
```
...
## 5. Detection（3D CA-CFAR + NMS + 峰值参数）

```text
1) 3D CA-CFAR：训练/保护窗（range, doppler, angle 三轴独立半径），边缘自适应计数
   threshold = noiseEst · α，α = Ktrain·(Pfa_cell^(-1/Ktrain) − 1)   （CA-CFAR 标准式）
2) NMS：局部极大（3×3×3 邻域）+ 贪心抑制（抑制半径=保护半径），按功率降序保留
3) 峰值插值：log 功率三轴抛物线插值（range/doppler/angle）
4) 几何换算：斜距→水平距离 r_h = sqrt(r²−h²)；φ = boresight_b + asin(u)
   局部笛卡尔 p = s_b + r_h·[cos φ, sin φ]
5) 质量字段：peak_power / noise_floor / peak_to_noise_db / cfar_score / 训练单元数
```

- **目标数量** = NMS 后的检测数（再经跨 BS 关联成观测），任何阶段不得来自 GT。
- **虚警**：由 `Pfa_cell` 决定，`Pfa_cell = target_false_alarms_per_bs_frame / N_cells`。
  注意 `N_cells = 512·512·64 ≈ 1.68e7`，若按 1e-4/cell 会得到 ~1677 虚警/帧，必须按目标虚警数反算
  （沿用旧 detector 的 `nominal_pfa=0.1/count` 思路；默认目标 ≈0.1–1 虚警/BS/帧，需标定）。
...
```text
1) 门控：任意两个不同 BS 的检测 (b1,b2)，位置差 d = p1−p2，S = C1+C2
   d²_M = dᵀ S⁻¹ d ≤ gate_χ²(2, 0.99) = 9.21
2) 图分量：把所有有门控边的检测连成连通分量（每 BS 至多 max_candidates 个）
3) 分量内匹配：
   - 恰一个 BS 一个检测 → 直接成观测（单 BS）
   - 每个检测至多用一次；枚举/匈牙利求"最大基数、最小总 Mahalanobis 代价"的一对一匹配
     （分量规模小，≤32³ 门控后极小；实现可调用 scipy.optimize.linear_sum_assignment）
   - 匹配对 ≥2 BS → 融合观测；落单检测 → 单 BS 观测
4) 融合：C_f = (Σ_b C_b⁻¹)⁻¹，p_f = C_f Σ_b C_b⁻¹ p_b；记录 bs_mask 与 n_bs
5) 输出：每帧 fused observation 列表 {x,y,C,bs_mask,n_bs,quality,time_ns}
```

- 不强设"参考传感器"；无匹配的检测不丢弃，以更大协方差进入跟踪。
- 单 BS 观测允许更新既有航迹；**单 BS 观测不能单独确认新航迹**（见 §7 确认规则）。
- 几何覆盖率支撑该策略：A01 统计 ≥2 BS 可见率 99.95%（全 3 BS 约 18%，典型为 2 BS）。
- 门限来自测量协方差（χ²），**不得来自 GT**；标定只允许用无目标噪声 CPI。
