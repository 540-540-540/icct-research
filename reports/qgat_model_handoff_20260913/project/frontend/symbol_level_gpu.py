...
def bounded_range_least_squares(ranges, stations, initial, bounds, max_iterations=40):
    ...
    for iteration in range(max_iterations):
        ...
        newradius = torch.linalg.vector_norm(candidate[:, None]-stations[None], dim=-1)
        displacement = candidate-p
        # Stable difference of distances: avoid subtracting two nearly equal
        # rounded objectives when the Gauss-Newton step is close to convergence.
        radius_change = (2*(delta*displacement[:, None]).sum(-1)+displacement.square().sum(-1)[:, None])/(newradius+radius)
        cost_change = (2*residual*radius_change+radius_change.square()).sum(-1)
        accepted = (cost_change < 0) & active
        change = (candidate-p).abs().amax(-1)
        # Tiny accepted progress is a convergence criterion, never a failed trial.
        converged |= accepted & ((change <= tolerance*(1+p.abs().amax(-1))) | (cost_change.abs() <= tolerance*tolerance*(1+cost)))
