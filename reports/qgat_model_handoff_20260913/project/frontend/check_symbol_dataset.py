...
def check_loader_and_normalization(data):
    ...
    # Extended-precision independent reference avoids sequential float64 reduction
    # error over millions of rows; keep the original absolute acceptance limit.
    reference_std=np.array([values[:,c].astype(np.longdouble).std() for c in range(4)],dtype=np.float64)
    np.testing.assert_allclose(normal['std'],np.maximum(reference_std,1e-3),rtol=0,atol=1e-12)
