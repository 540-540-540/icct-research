...
from prediction.raj_qgnn.metrics import trajectory_metrics
from prediction.raj_qgnn.common import physical_graph
...
def interaction_stats(history,mask):
    ...
    last=edge[:,:,:,0:2]
    n=mask.shape[1]
    upper=torch.triu(torch.ones(n,n,device=mask.device,dtype=torch.bool),diagonal=1)[None]
    pair=mask[:,:,None]&mask[:,None,:]&upper
    closing=((last[...,0]<1.0)&(last[...,1]>0.05)&pair).sum((1,2))
    max_risk=torch.where(pair,risk,torch.zeros_like(risk)).amax((1,2))
    return {'closing_pairs_30m_gt_0p5':closing,'max_pair_risk':max_risk,
...
def evaluate(model,loader,device):
    ...
    for batch in loader:
        ...
        for j in range(h.shape[0]):
            rows.append({'index':index+j,'scene_id':int(batch['scene_id'][j]),'start_frame':int(batch['start_frame'][j]),'ADE':float(metric['scene_ade'][j]),'FDE':float(metric['scene_fde'][j]),'closing_pairs_30m_gt_0p5':int(stats['closing_pairs_30m_gt_0p5'][j]),'max_pair_risk':float(stats['max_pair_risk'][j]),'vehicle_count':int(stats['vehicle_count'][j])})
        index+=h.shape[0]
...
def main():
    ...
    p.add_argument('--kind',choices=['quantum','matched_classical','gin','ppgn'],required=True)
    ...
    p.add_argument('--classical-hidden',type=int,default=42)
    ...
    source={str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in (ROOT/'prediction/raj_qgnn').glob('*.py')}
    source['scripts/train_raj_qgnn.py']=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    token_hash=hashlib.sha256((ROOT/'configs/raj_qgnn_tokens.json').read_bytes()).hexdigest()
    config=dict(vars(args));config.update({'source_sha256':source,'token_sha256':token_hash,'pid':os.getpid(),'cuda_visible_devices':os.environ.get('CUDA_VISIBLE_DEVICES'),'git_head':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip(),'test_set_used':False})
    if not args.resume: atomic_json(outdir/'config.json',config)
    model=build_model(args.kind,args.seed,args.depth,correction_cap_m=args.correction_cap,
                      classical_hidden=args.classical_hidden).to(device)
    ...
    if args.resume:
        ...
        for key in ['kind','seed','snr','epochs','batch_size','train_limit','depth','classical_hidden','lr','correction_cap']:
