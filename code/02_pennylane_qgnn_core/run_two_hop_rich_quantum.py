"""Mechanism-informed rich-observable quantum run on frozen two-hop scenes."""
import hashlib
import json
import math
import platform
...
from rich_observable_core import RichObservablePQC
...
OUTPUT = BASE / "two_hop_rich_quantum_seed2026_v1"
...
class RichSelectiveLayer(CoreMessageLayer):
    def __init__(self, config):
        super().__init__(config, "quantum", quantum_backend="pennylane")
        original_weights = self.core.weights.detach().clone()
        with torch.random.fork_rng(devices=[]):
            torch.manual_seed(2028)
            rich = RichObservablePQC(QuantumCircuitConfig())
        with torch.no_grad():
            rich.weights.copy_(original_weights)
        self.core = rich
...
def build_model(rich):
    torch.manual_seed(SEED)
    config = ForecasterConfig(graph_radius_m=20.0)
    model = TargetInteractionGNN(config)
    torch.manual_seed(SEED + 1)
    model.graph_layers[1] = RichSelectiveLayer(config) if rich else CoreMessageLayer(
...
def main():
    ...
    # Prove exact Z/ZZ core parity before the score-temperature intervention.
    r.set_seed(SEED)
    ...
    loss.backward()
    extra_gradient = float(candidate.graph_layers[1].core.readout.weight.grad[:, 12:].abs().max())
    assert extra_gradient > 0
    smoke = {
        "baseline_core_initial_parity_max_error": parity_error,
        "extra_observable_readout_gradient_max": extra_gradient,
    ...
    source_paths = [Path(__file__), BASE / "rich_observable_core.py", BASE / "model.py",
    ...
    protocol = {
        ...
        "intervention": {
            ...
            "raw_observables": 36,
            "learned_projection": "36 to 12, initialized to exact retained Z/ZZ readout",
            "attention_score_scale": 2.0,
        },
    ...
    audit = train_stage(model, "quantum_rich_radius20", "graph", 40, 6, train, selection, OUTPUT, expected)
    confirmation_metrics, arrays = retained.evaluate(model, confirmation, collect=True)
    np.savez_compressed(
        OUTPUT / "quantum_rich_radius20_graph_confirmation.npz",
