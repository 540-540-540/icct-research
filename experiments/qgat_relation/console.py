...
def estimate(progress, live_step_seconds=None, validation_elapsed=0.):
    ...
    if INHERITED_EPOCHS and step_time is None:
        basis = '等待续训阶段新耗时'
...
class CellView:
    ...
    def snapshot(self, now=None):
        ...
        if complete:
            ...
            waiting = f'等待从{INHERITED_EPOCHS}轮续训' if INHERITED_EPOCHS and not p else '等待启动'
...
def render(rows, owner_running=False):
    ...
    if all_complete:
    ...
    elif not active and not any(row['progress'] for row in rows):
        lines.append(f'整体状态：等待从{INHERITED_EPOCHS}轮断点续训；首次启动会建立独立分支。'
                     if INHERITED_EPOCHS else '整体状态：等待从头训练。')
    elif not active:
        ...
        validation_wait = '续训首轮验证完成后估算' if INHERITED_EPOCHS else '首轮验证完成后估算'
    ...
    for row in rows:
        ...
        device = row['job'].get('device', DEVICES.get(row['cell'], '—'))
        lines += ['', f'{CELLS[row["cell"]]}  |  {device}  |  {status}']
        if INHERITED_EPOCHS and not p:
            lines.append(f'  等待载入{INHERITED_EPOCHS}轮父断点；目标共{EPOCHS}轮，续训{EPOCHS-INHERITED_EPOCHS}轮。')
            continue
        ...
        if eta['total'] is None:
            validation_wait = '续训首轮' if INHERITED_EPOCHS else '首轮'
...
def main():
    global CELLS, REPORT, RESULTS, RUNNER, TITLE, EPOCHS, INHERITED_EPOCHS, DEVICES, BATCHES, TOTAL_STEPS
    ...
    models.add_argument('--value', action='store_true', help='fresh paired QGAT run: relation matching vs relation messages, 100 epochs, global batch 16, one GPU each')
    args = p.parse_args()
    ...
    RUNNER = Path(__file__).with_name('run.py')
    TITLE = 'QGAT 原 D / 新 D · 40 轮配对训练 · seed 2026'
    EPOCHS = 100 if args.extend100 or args.value else 40
    INHERITED_EPOCHS = 40 if args.extend100 else 0
    DEVICES = {}
    BATCHES = math.ceil(ORIGINS/BATCH)
    TOTAL_STEPS = EPOCHS*BATCHES
    if args.value:
        CELLS = {'relation_D': '当前 QGAT（关系匹配）', 'relation_value': 'QGAT（关系匹配＋关系消息）'}
        REPORT = ROOT/'reports/qgat_value/seed2026'
        RESULTS = ROOT/'results/qgat_value/seed2026'
        RUNNER = ROOT/'experiments/qgat_value/run.py'
        TITLE = 'QGAT 新鲜配对 · 从头训练 100 轮 · seed 2026 · 总批次 16 · 各一 GPU（基线 0 / 关系消息 1）'
        DEVICES = {'relation_D': 'cuda:0', 'relation_value': 'cuda:1'}
    elif args.extend100:
