...
for seed in 2026 2027; do
  for spec in "075:0.75" "100:1.0"; do
    ...
    CUDA_VISIBLE_DEVICES=$gpu "$PYTHON" -u experiments/gate_b_plus/run_screen.py \
      --mode full_multiscale --seed "$seed" --fde-weight "$weight" --epochs 30 --resume --output "$output" \
      > "${output}_extension.log" 2>&1 &
    pids+=("$!"); index=$((index + 1))
  done
  gpu=$((index % 2))
  output="reports/task_redesign/sind_gate_b_plus_screen_johnson_full_multiscale_seed${seed}"
  CUDA_VISIBLE_DEVICES=$gpu "$PYTHON" -u experiments/gate_b_plus/run_screen.py \
    --mode full_multiscale --core-kind johnson --seed "$seed" --epochs 30 --resume --output "$output" \
    > "${output}.log" 2>&1 &
  pids+=("$!"); index=$((index + 1))
done
status=0
