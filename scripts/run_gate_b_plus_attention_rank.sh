...
for seed_gpu in "2026:0" "2027:1"; do
  seed=${seed_gpu%%:*}
  gpu=${seed_gpu##*:}
  for mode in quantum_latent_attention multiscale_quantum_attention; do
    output="reports/task_redesign/sind_gate_b_plus_${mode}_rank005_seed${seed}"
    CUDA_VISIBLE_DEVICES=$gpu "$PYTHON" -u experiments/gate_b_plus/run_screen.py \
      ...
      --latent-rank-weight 0.05 --output "$output" > "${output}.log" 2>&1 &
    pids+=("$!")
  done
done
status=0
...
exec bash scripts/run_gate_b_plus_dual_readout.sh
