...
SceneAdaptiveQuantumCore=partial(_QuantumCore,phase_mode=True)
...
OUT=ROOT/"reports/qgnn/round3_engineering_v2.json"
...
def reference():
    ...
    feedback=None;ps=[];ts=[]
    for l in range(3):
        l2,l3=q.controller(l,d,feedback)
        active=m.sum(1).to(h.dtype).clamp_min(1)
        n2=(d["pw"].square().sum(1)/active).clamp_min(1).sqrt()
        n3=(d["tw"].square().sum(1)/active).clamp_min(1).sqrt()
        ps.append(pa[:,l]+(1.5*(l2-1)*d["pw"][:,None]/n2[:,None,None]).reshape(ch,-1))
        ts.append(ta[:,l]+(1.5*(l3-1)*d["tw"][:,None]/n3[:,None,None]).reshape(ch,-1))
...
def profile():
    ...
    for kind in ["quantum","classical"]:
        model=build_model(kind,adaptive_mode="phase_feedback",controller_init="specialized").cuda();digest=hashlib.sha256()
