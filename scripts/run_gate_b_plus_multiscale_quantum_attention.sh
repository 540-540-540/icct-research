...
for seed_gpu in "2026:0" "2027:1"; do
  seed=${seed_gpu%%:*}
  gpu=${seed_gpu##*:}
  output="reports/task_redesign/sind_gate_b_plus_multiscale_quantum_attention_seed${seed}"
  CUDA_VISIBLE_DEVICES=$gpu "$PYTHON" -u experiments/gate_b_plus/run_screen.py \
    --mode multiscale_quantum_attention --seed "$seed" --order-seed 20260921 --dropout-seed 20260921 \
