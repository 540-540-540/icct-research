...
from prediction.qgnn_adaptive.quantum import SceneAdaptiveQuantumCore as _QuantumCore
from functools import partial
SceneAdaptiveQuantumCore=partial(_QuantumCore,basis_feedback=True)
from prediction.qgnn_adaptive.classical import SceneAdaptiveClassicalCore as _ClassicalCore
SceneAdaptiveClassicalCore=partial(_ClassicalCore,basis_feedback=True)
...
OUT=ROOT/"reports/qgnn/round3_engineering_v3.json"
...
def reference():
    ...
    def circuit(iy,iz,ip,it,ix):
        for l in range(iy.shape[0]):
            ...
            for j in range(n):qml.RX(ix[l,j],j)
        return qml.state()
    _,z,pp,tt,zz,zzz=tables(n,h.device,h.dtype)
    feedback=None;ps=[];ts=[];mixes=[]
    for l in range(3):
        l2,l3=q.controller(l,d,feedback);ps.append(pa[:,l]*l2.reshape(ch,-1));ts.append(ta[:,l]*l3.reshape(ch,-1))
        mix=rx[:,l,None].expand(-1,n)
        if l>0:
            pool=[]
            for fv,fw,fi in zip(feedback,[d["pw"],d["tw"]],[d["pairs"],d["tri"]]):
                for order in [1,2]:
                    bynode=[]
                    for node in range(n):
                        sel=(fi==node).any(-1);wt=fw[:,sel]
                        bynode.append(((fv[:,:,sel]**order)*wt[:,None]).sum(-1)/wt.sum(-1)[:,None].clamp_min(1e-6))
                    pool.append(torch.stack(bynode,-1))
            nf=torch.stack(pool,-1)
            mix=mix+.6*torch.tanh((nf*q.basis_weights[:,l-1][None,:,None]).sum(-1)).reshape(ch,n)
        mixes.append(mix)
        ip=torch.stack(ps,1);it=torch.stack(ts,1)
        refs=torch.stack([circuit(ry[c,:l+1],rz[c,:l+1],ip[c],it[c],torch.stack(mixes,1)[c]) for c in range(ch)])
...
def profile():
    ...
    for kind in ["quantum","classical"]:
        model=build_model(kind,adaptive_mode="basis_feedback",controller_init="specialized").cuda();digest=hashlib.sha256()
