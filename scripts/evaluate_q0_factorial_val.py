"""Validation-only evaluator for 2x2 factorial models."""
...
def load(model,path):
    ...
    missing,unexpected=model.load_state_dict(p['model_state'],strict=False)
    if unexpected or any(not k.startswith('temporal.llm.') for k in missing):
...
def main():
    ...
    model=build_factorial_model(a.graph,a.downstream,stats,init_seed=a.seed).to(device)
    payload=load(model,a.checkpoint)
    model.eval()
    ...
    with torch.no_grad():
      for b in dl:
        ...
        for j in range(h.shape[0]):
          ...
          rows.append(r)
        cur+=h.shape[0]
    s={'model':f'{a.graph}_{a.downstream}','snr_db':a.snr,'checkpoint_validation':payload.get('validation'),
    ...
    out.write_text(json.dumps({'summary':s,'rows':rows},indent=2)+'\n')
