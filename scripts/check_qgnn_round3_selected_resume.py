...
OUT=ROOT/'reports/qgnn/round3_selected_resume_regression.json'
def save():
    t=OUT.with_suffix('.tmp');t.write_text(json.dumps(report,indent=2)+'\n');t.replace(OUT)
base=[sys.executable,'scripts/train_qgnn_adaptive.py','--kind','quantum','--seed','2299','--snr','0','--epochs','1','--batch-size','16','--train-limit','64','--quantum-version','3','--correction-cap','16','--adaptive-mode','phase_feedback']
straight='reports/qgnn/round3_selected_resume_straight_20260919'
interrupted='reports/qgnn/round3_selected_resume_interrupted_20260919'
try:
