"""Fixed-protocol feedback on/off retraining ablation; not model selection."""
...
def main():
    ...
    p.add_argument('--kind',choices=['quantum','classical'],required=True)
    p.add_argument('--feedback-training',choices=['on','off'],required=True)
    ...
    p.add_argument('--controller-init',choices=['specialized','neutral'],default='neutral')
    ...
    source.update({str(f.relative_to(ROOT)):hashlib.sha256(f.read_bytes()).hexdigest() for f in (ROOT/'prediction/qgnn_adaptive').glob('*.py')})
    source['scripts/train_qgnn_feedback_ablation.py']=hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
    source['scripts/train_qgnn_adaptive.py']=hashlib.sha256((ROOT/'scripts/train_qgnn_adaptive.py').read_bytes()).hexdigest()
    for f in ['prediction/q0/motion_token_llm.py','prediction/q0/temporal.py','prediction/q0/metrics.py','scripts/train_q0_motion_llm.py','frontend/sind_prediction_dataset.py']:
    ...
    if args.adaptive_mode!='phase_feedback' or args.controller_init!='neutral':
        raise ValueError('Ablation is frozen to phase_feedback + neutral')
    model.graph.feedback_enabled=args.feedback_training=='on'
    initial_digest=hashlib.sha256()
    for name,param in model.named_parameters():
        if param.requires_grad:
            initial_digest.update(name.encode());initial_digest.update(param.detach().cpu().numpy().tobytes())
    ...
    if args.resume:
        ...
        for key in ['kind','seed','snr','epochs','batch_size','train_limit','depth','lr','channels','quantum_version','correction_cap','adaptive_mode','controller_init','feedback_training']:
    ...
    def summary(status):
        atomic_json(outdir/'summary.json',{'status':status,'kind':args.kind,'feedback_training':args.feedback_training,'initial_mutable_sha256':initial_hash,'best_validation':best,'best_epoch':best_epoch,'epochs_completed':len(records),'global_step':global_step,'elapsed_seconds':elapsed(),'parameters':model.parameter_summary(),'shared_initialization_sha256':shared_hash,'train_indices_sha256':hashlib.sha256(selected.tobytes()).hexdigest(),'train_samples':len(train),'validation_samples':len(val),'test_set_used':False})
